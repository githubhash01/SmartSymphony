import pyaudio
from numpy import zeros, linspace, short, fromstring, hstack, transpose, log, log2, abs
...
class NotesHz():
    # Define the frequencies for each note - only non-sharp/flat notes included for brevity
    C2  = 65.406
    D2  = 73.42
    E2  = 82.41
    F2  = 87.31
    ...
    def process_samples(self):
        while self.running or not self.buffer.empty():
            if not self.buffer.empty():
                ...
                if freq is not None and intensity > Values.MIN_INTENSITY:
                    detected_note = frequency_to_note(freq)
                    print(f"Detected frequency: {freq:.2f} Hz, Detected note: {detected_note}, Intensity: {intensity:.2f}")
