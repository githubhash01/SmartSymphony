...
class Tuner():
    ...
    def collect_samples(self):
        while self.running:
            audio_data = np.frombuffer(self.stream.read(Values.NUM_SAMPLES, exception_on_overflow=False), dtype=short)
            ...
            if not self.buffer.empty():
                ...
                if len(audio_samples) == Values.SAMPLES_FOR_AVERAGE:
                    ...
                    if (vals != None and len(vals) != 0):
                        #print("left micropohone signal detected",time.time())
                        for val in vals:
