...
class Tuner():
    ...
    def get_frequency_from_samples_right(self,audio_samples):
        ...
        for audio_data in audio_samples:
            ...
            if theintensity >= Values.RIGHT_MIN_INTENSITY:
    ...
    # Zip and filter the results
        ...
        zipped = [(a, b) for a, b in zipped if b >= Values.RIGHT_MIN_INTENSITY and a >= Values.RIGHT_MIN_FREQUENCY]
        if len(zipped) == 0:
            return []
    ...
    def get_frequency_from_samples_left(self,audio_samples):
        ...
        for audio_data in audio_samples:
            ...
            theintensity = intensity[max_intensity_index]
            if theintensity <= Values.LEFT_MAX_INTENSITY and Values.LEFT_MIN_INTENSITY <= theintensity:
                frequencies.append(thefreq)
                intensities.append(theintensity)

    # Zip and filter the results
        zipped = list(zip(frequencies, intensities))
        zipped = [(a, b) for a, b in zipped if b <= Values.LEFT_MAX_INTENSITY and Values.LEFT_MIN_INTENSITY <= b and Values.LEFT_MAX_FREQUENCY >= a and Values.LEFT_MIN_FREQUENCY <= a]
        if len(zipped) == 0:
            return []
    ...
    # Group frequencies based on a threshold gap
        ...
        for i in range(0,len(zipped)):
            if current_group == None:
                if zipped[i][0] > Values.LEFT_MAX_FREQUENCY:
                    continue
                else:
                    current_group = [zipped[i]]
                ...
                if len(audio_samples) == Values.SAMPLES_FOR_AVERAGE:
                    ...
                    if(vals != None and len(vals) != 0):
                        for val in vals:
                            ...
                            if(right == True) and (count <= Values.WAIT_CYCLE_TIME):
                                ...
                                started = False
                                count = 0
                    if count >=Values.WAIT_CYCLE_TIME and len(temp) != 0:
                        for note,t in temp:
                            print("note detected "+note+" by right" )
                            print(t)
                            self.results.append((note,t))
                        temp = []
                        count = 0
                        started = False
