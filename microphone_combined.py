...
class Values():
    ...
    PRECISION = 0.8
    SAMPLES_FOR_AVERAGE = 3 #5(this worked)  # Number of samples to average
    RIGHT_MIN_FREQUENCY = 115
    RIGHT_MAX_FREQUENCY = 1150
    RIGHT_MIN_INTENSITY = 75
    LEFT_MAX_INTENSITY = 30 #30(worked)
    LEFT_MIN_INTENSITY = 5 #5 worked
    LEFT_MIN_FREQUENCY = 40
    LEFT_MAX_FREQUENCY = 120
    Frequency_Difference_Threshold = 100
    WAIT_CYCLE_TIME = 5 #5 worked
...
class Tuner():
    ...
    def get_frequency_from_samples(self,audio_samples):
        ...
        for audio_data in audio_samples:
            ...
            if (theintensity >= Values.RIGHT_MIN_INTENSITY) or theintensity <= Values.LEFT_MAX_INTENSITY and Values.LEFT_MIN_INTENSITY <= theintensity:
                frequencies.append(thefreq)
                intensities.append(theintensity)
            zipped = list(zip(frequencies, intensities))
            zipped = [(a, b) for a, b in zipped if (b >= Values.RIGHT_MIN_INTENSITY and a >= Values.RIGHT_MIN_FREQUENCY) or (b <= Values.LEFT_MAX_INTENSITY and Values.LEFT_MIN_INTENSITY <= b and Values.LEFT_MAX_FREQUENCY >= a and Values.LEFT_MIN_FREQUENCY <= a)]
            if len(zipped) == 0:
                return []
            ...
            for i in range(0,len(zipped)):
                if current_group == None:
                    if (zipped[i][0] > Values.LEFT_MAX_FREQUENCY and zipped[i][0] < Values.RIGHT_MIN_FREQUENCY) or (zipped[i][0] > Values.RIGHT_MAX_FREQUENCY):
                        continue
                    else:
                        current_group = [zipped[i]]
                elif (zipped[i][0] > Values.LEFT_MAX_FREQUENCY and zipped[i][0] < Values.RIGHT_MIN_FREQUENCY) or (zipped[i][0] > Values.RIGHT_MAX_FREQUENCY):
                    continue
            ...
            if not self.buffer.empty():
                ...
                if (vals != None and len(vals) != 0):
                    ...
                    for val in vals:
                        ...
                        print("detected note is",detected_note)
