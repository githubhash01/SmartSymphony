...
class Values():
    ...
    PRECISION = 0.8
    SAMPLES_FOR_AVERAGE = 5  #5(this works)  # Number of samples to average
...
class Tuner():
    ...
    # Zip and filter the results
        zipped = list(zip(frequencies, intensities))
        zipped = [(a, b) for a, b in zipped if b >= Values.MIN_INTENSITY]
        if len(zipped) == 0:
            return []
    ...
    # Group frequencies based on a threshold gap
        ...
        for i in range(1, len(zipped)):
            if (zipped[i][1] - zipped[i-1][1] > 0) or (zipped[i][0] - zipped[i-1][0] > 100):  # Threshold for a new group
